...
Checkpoint averaging, stronger stopping rules, and label-free selector diagnostics are complementary rather than direct substitutes.
These works mainly propose or evaluate alternative return rules; our object of study is different: we audit what reliability claim remains valid when a shaped training workflow reports the checkpoint selected by its own proxy.
Appendix Table~\ref{tab:camelyon-selector-averaging} adds a deliberately narrow same-trajectory averaging sensitivity on our SoftClip and GCE runs; it is not a SWAD or Ensemble-of-Averages benchmark, but checks whether simple averaging removes the proxy-selected reliability gap.

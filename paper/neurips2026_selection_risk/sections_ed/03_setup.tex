...
We evaluate checkpoints in two ways: a held-out loss on the deployment split (worst-group loss on CelebA, hospital-2 loss on Camelyon17, overall loss plus official WILDS worst-group loss on CivilComments, raw MSE on ACSIncome) and a tail-sensitive diagnostic (teacher-difficulty $\mathrm{CVaR}_{0.1}$ on the vision anchors, tail MSE on ACSIncome).
...
\end{equation}
where $R_w<1$ indicates relative tail suppression and $R_w>1$ indicates relative tail upweighting.
